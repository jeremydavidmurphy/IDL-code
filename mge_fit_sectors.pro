...
;           OUTER_SLOPE=outer_slope, ABSDEV=absdev
;
; INPUTS:
...
;       V4.01: Added output keyword ABSDEV. Fixed display not being updated 
...
ENDFOR
wait, 0.1 ; Fixes display not being updated under Windows.
...
PRO MGE_fit_sectors, radius1, angle1, counts1, eps, $
    ...
    OUTER_SLOPE=outer_slope, BULGE_DISK=bulge_disk, ABSDEV=absdev
COMPILE_OPT IDL2
;
...
ENDIF ELSE BEGIN
    ...
    IF N_ELEMENTS(sol) le 1 THEN BEGIN
    ...
    if keyword_set(bulge_disk) then begin
        parinfo[ngauss+2:*:2].tied = 'P['+strtrim(ngauss,2)+']' ; Ties axial ratio of even Gaussians
        parinfo[ngauss+3:*:2].tied = 'P['+strtrim(ngauss+1,2)+']' ; Ties axial ratio of odd Gaussian
    endif
...
PRINT, ' MEANABSDEV: ', STRTRIM(absdev,2)
