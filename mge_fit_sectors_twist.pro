...
;       V3.97: Changed definition of output PA to be positive in 
...
; axial ratio and PA, to have the same axial ratio and PA as the mean 
; of the first two well determined Gaussians. Flux is conserved by PSF
; convolution so no other changes are required
;
w1 = where(sol[1,*] le min(sigmaPSF), COMPLEMENT=w2,p)
if p gt 0 then begin
    sol[2,w1] = mean(sol[2,w2[0:1]])
    sol[3,w1] = mean(sol[3,w2[0:1]])
endif    
